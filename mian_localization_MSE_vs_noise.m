...
% completion and semidefinite programming, averaged MSE vs noise ratio  
...
nsim = 1e1; %number of monte carlo simulations
ratio_noise = 0.1:0.1:1;
alpha = 0.1;  %lost ratio

d = 2; %problem dimensions
N_a = size(u,2); %number of anchors
N_s = 10;  %number of sensor nodes

NMSE1 = zeros(1,length(ratio_noise));
NMSE2 = zeros(1,length(ratio_noise));

for r=1:length(ratio_noise)
    for ns=1:nsim
        ...
        Noise = ratio_noise(r) * norm(D1,'fro')/norm(Noise,'fro')*Noise;
        Do = D1;
        ...
        nmse_X_sdp = norm(x-X_hat,'fro')/norm(x,'fro');
        NMSE1(r) = NMSE1(r) + nmse_X_sdp;
...
figure(2)
clf
plot(ratio_noise,NMSE1,'-bo')
hold on
plot(ratio_noise,NMSE2,'-ro')
xlabel("Noise ratio")
ylabel("Normalized mean squared error(NMSE)")
legend("localization by low-rank matrix completion + SDP","localization by low-rank matrix completion + triangulation")
ttl = strcat("IoT localization NMSE with lost ratio =",num2str(alpha));
title(ttl)
